...
The supervisor uses one structured LLM decision for routing and durable-memory
extraction. It controls direct specialist routing, handoffs/fan-in, and unified
final-response synthesis without duplicate intent classifiers or a reviewer layer.
...
$env:SMARTDESK_MODEL="google/gemma-4-31b-it:free"
$env:TAVILY_API_KEY="..."
python graph.py
```
...
```text
Supervisor Plan -> Memory (semantic top-3 + history trimming every turn)
...
Parallel ResearchAgent and TaskAgent branches join exactly once. Research/Task/Join
may hand off to WriterAgent; every successful path ends at supervisor-owned
completion. Agent/model failures route to a graceful Error node and then END.
